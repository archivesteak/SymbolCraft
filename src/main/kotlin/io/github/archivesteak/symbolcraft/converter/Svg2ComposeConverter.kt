...
class Svg2ComposeConverter {
    ...
    private fun makeDeterministic(
        ...
        val header = SymbolCraftDefaults.GENERATED_FILE_HEADER
        return content
            // Remove timestamp comments (common patterns)
            .replace(Regex("//.*Generated on.*\\d{4}-\\d{2}-\\d{2}.*"), header)
            .replace(Regex("//.*Created on.*\\d{4}-\\d{2}-\\d{2}.*"), header)
            .replace(Regex("//.*Date:.*\\d{4}-\\d{2}-\\d{2}.*"), header)
            .replace(Regex("//.*\\d{4}-\\d{2}-\\d{2}\\s+\\d{2}:\\d{2}:\\d{2}.*"), header)
            // Remove specific svg-to-compose timestamp patterns
            .replace(Regex("//\\s*Generated by svg-to-compose.*"), header)
            .replace(Regex("//\\s*Converted from.*\\d{4}-\\d{2}-\\d{2}.*"), header)
            ...
            // Stamp every file as ours so cleanup can safely distinguish generated sources from
            // hand-written Kotlin files living in the same output tree.
            .let { ensureGeneratedHeader(it) }
    }

    /** Guarantees the file's first line is the SymbolCraft marker. */
    private fun ensureGeneratedHeader(content: String): String {
        val header = SymbolCraftDefaults.GENERATED_FILE_HEADER
        return if (content.startsWith(header)) content else "$header\n$content"
    }
